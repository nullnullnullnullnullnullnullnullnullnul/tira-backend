...
## Task deletion leaves no surviving audit trail (by design)

**Decision**: when a task is deleted, every audit row associated
with it is also deleted, including any row generated by the delete
itself. There is no surviving "task X was removed by user Y at
time T" record in `task_history` or anywhere else.

**Why this happens**: `task_history.task_id` is declared
`FK ... ON DELETE CASCADE` in
[`migrations/0008_create-task-history.sql`](../../migrations/0008_create-task-history.sql).
Deleting a row from `tasks` therefore propagates a DELETE into
`task_history` that removes every historical row for that task.
The same transaction also fires `log_tag_activity_fn` for each
CASCADE-driven `task_tags` removal, which inserts a fresh
`task_history` row attributed to the actor; that row is then
cascade-deleted along with the rest. Net result: an outside reader
sees the task and its full history simply vanish.

**Why `deleteTask` is wrapped in `withAuditedTransaction` anyway**:
see the commit message for `feat(service): wrap deleteTask in an
audited transaction`. The wrap is defensive (correct attribution
in the brief in-transaction window, consistent pattern for
contributors, becomes load-bearing if the FK rule below ever
changes), not corrective.

**What it would take to actually fix the gap**: two changes in
concert.

1. Change the FK on `task_history.task_id` from
   `ON DELETE CASCADE` to `ON DELETE SET NULL` (and make `task_id`
   nullable). Existing audit rows then survive task deletion as
   orphaned-but-readable rows.
2. Add an `AFTER DELETE` trigger on `tasks` that inserts a final
   `change_type='DELETE', entity='TASK'` row into `task_history`,
   carrying the OLD task_id in a new column like
   `deleted_task_id TEXT` (because the regular `task_id` column
   would be NULL'd by step 1's SET NULL on the same row).

Neither step is small, and "the audit trail follows the task"
(today's behaviour) is internally consistent: every other
task-owned entity (`comments`, `task_tags`) also disappears with
the task. Recording the deletion separately would single out
`task_history` as the one exception, which is a defensible
design but a real one.

**Recorded as a known limitation** rather than fixed because the
delete path is rare in normal use and the surviving artefact
(the task simply not being there) is a clear-enough signal that
something was removed. If the audit log ever needs to satisfy a
compliance or forensics requirement, revisit; for portfolio /
local use the current behaviour matches the rest of the cascade
design.
