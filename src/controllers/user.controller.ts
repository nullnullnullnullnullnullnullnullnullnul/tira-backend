...
export async function listUsers(req: Request, res: Response) {
  try {
    ...
    else throw new Error("No users found");  // Tendria que estar en un middleware
  } catch (e) {
    console.error(e);
    res.status(500).json({
...
export async function getUser(req: Request, res: Response) {
  try {
    ...
    if(!users) throw new Error("User not found"); // middleware
    res.json(users);
  } catch(e) {
    console.error(e);
    res.status(500).json({
...
export async function createUser(req: Request, res: Response) {
  try {
    ...
    if(!newUser) throw new Error("Invalid role");  // middleware
...
export async function deleteUser(req: Request, res: Response) {
  try {
    ...
    if (!deleted) throw new Error("User not found"); // middleware
